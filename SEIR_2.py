...
IT = np.array([0, 0, 0.05, 0.1, 0.15, 0.3, 0.5]) # Incubation time 
TT = np.array([1]) # Test time
IBST = np.array([1]) # IBST(tau) = P(infective at day of symptoms - tau)
...
def estimate_parameters(U, dU, **kwargs):
    ...
    if alpha==None and beta==None and gamma==None and epsilon==None:       #estimate all parameters
        ...
        print('Specify either alpha gamma and epsilon or no parameters at all\n')
        param = None
        Rsq = None 
        return 
...
def solve_SEIR(U, k=1, resolution=1, **kwargs):
...
def logistic_fit(U):
    ...
    t = range(len(I))
    param_l, cov_l = curve_fit(logistic, t, I, guess)
