...
The percentage indicator is calculated dividing the total number of Covid cases by the population of the respective region/province (the latter retrieved from Istat \cite{Istat+res0,Istat+res1}).
\begin{equation}
...
The summary statistics section (Fig. \ref{Home_fig3}) provides some barometer level statistics regarding the current total number of COVID19 cases in Italy with three further breakouts: intensive care, hospitalised, and home isolation.
...
Panel \textit{Analysis} explores different mathematical models for shaping the infections time series, namely the ones introduced in \ref{Theoretical Background}. As previously argued, each of these models predictive power hinges on uncertain actual scenarios, such as the effectiveness of restrictive measures, the presence of several independent and spread outbreaks, and so on. Package \cite{growthcurver} has been used for growth curve fitting. This package provides function \textit{SummarizeGrowth}, which inputs the time series of sample data and outputs the model represented by the growth metrics. Such parameters include those of the logistic equation that best fit the data \ref{logistic_equation}. The reasons why this package was chosen are chiefly two: at the one hand, it performs non-linear curve fitting with the Levenberg-Marquardt algorithm \cite{more1978levenberg}, which happens to be one of the most robust non-linear least squares  (NLS) algorithms. For this step, function \cite{nlsLM} is used. On the other hand, the package is endowed with two methods for background correction.

At the top of analysis panel the user is allowed to pick one dataset among national, by region and by province. Hence, any following analysis will use the selected dataset as input (the user can browse the raw dataset in panel inspection, see \ref{Data Inspection} ). The first section is devoted to the logistic model. An input box \ref{fig:logistic_input} lists several dynamic inputs: 
\begin{itemize}

\item The date range that the calculator will use for curve fitting. The option to choose initial and final dates different from those at, resepectively, the beginning and the end of the dataset, is useful in two ways: first, it allows to cut initial dates which possibly correspond to zero or constant infections data, as a consequence of a delay in the outbreak in that territory; second, it allows to compute the availabe prediction n days ahead, by cutting the last n dates, and compare it to the real data. \item A check-box that, if selected, will use for computations and rendering the data of the share of infected among the tested, instead of the the infected themselves. That is, it will "standardise positive cases by total swabs". This may be useful to capture the "selection bias", but only in the scenario in which tests are randomly assigned to the population, or at least are assigned with a logic common to all dates and territories. This option is not currently available for provinces. \item Plot type selection boxes: should cumulative cases and fitted curve be plotted or new cases, or both (The user can always show and hide plots' components by clicking on their label in the legend).
\item Residuals plot type. Four options for graphically rendering of the residuals are available: Residuals vs. fitted values; Standardized residuals vs. fitted values; Autocorrelation; Square root of absolute residuals vs. fitted values. Output is displayed within three boxes: Inside the \textit{Plot} box \ref{fig:logistic_plot1} the sample points, the fitted logistic curve and a confidence band at 95 \% level for the logistic curve are overlayed. The user can view the sample and fitted values by hovering over or touching (for touch-screen devices) them. In addition, if \textit{new cases} is selected, a plot of sample cases differences (which correspond to new cases, in fact) is shown over the logistic estimated distribution \ref{fig:logistic_plot2}. The latter is simply obtained by deriving the right hand side of formula \ref{logistic_equation}
\end{itemize}
\begin{equation}
\dot{N}= \frac{(\frac{K-N_0}{N_0})Kre^{-rt}}{[1+(\frac{K-N_0}{N_0})e^{-rt}]^2}
\end{equation}
